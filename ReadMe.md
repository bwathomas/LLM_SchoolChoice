...
This program automates the querying of the OpenAI ChatGPT API to collect and analyze information about the kind of school choice rankings ChatGPT provides based on prompt details. The program reads templates and variable values from Excel files, processes permutations of these variables, queries the API, extracts and matches school names, and generates detailed reports and analyses.
...
- [Matrix Analysis Functions](#matrix-analysis-functions)
  ...
  - [Checking Columns and Variable Names](#checking-columns-and-variable-names)
  - [Creating Symmetrical Matrices](#creating-symmetrical-matrices)
  - [Adding Statistics to Matrices](#adding-statistics-to-matrices)
  - [Performing T-Tests and PCA](#performing-t-tests-and-pca)
...
school_points_fraction = school_points_fraction.xlsx
...
- `school_points_fraction`: Name of the Excel file containing school points fractions.
...
To run the matrix analysis program, execute:

```sh
python MatrixAnalysis.py
```

The `DataCollector.py` program will read the configuration, process permutations, query the OpenAI API, extract and match school names, and save the results to Excel files. The `MatrixAnalysis.py` program will perform a detailed statistical analysis on the results generated by the `DataCollector.py`.

In the event you are interested in changing the prompt, simply change `prompt_template.txt`. Words in brackets (such as `[income]`) in the prompt will be substituted by variables from `variables.xlsx`. To change the schools considered valid answers by the program, modify `school_list.xlsx`.
...
### Checking Columns and Variable Names
...
The `MatrixAnalysis.py` main function performs the following steps:

1. Loads configuration settings.
2. Loads the data from the results file.
3. Identifies school columns and variable columns.
4. Creates variable names and adds a column for them.
5. Creates symmetrical matrices for each school and adds statistical data.
6. Writes individual school matrices to Excel.
7. Computes the average matrix and adds statistical data.
8. Performs t-tests and PCA on the average matrix.
9. Writes the average matrix, t-test results, and PCA results to separate Excel files.
